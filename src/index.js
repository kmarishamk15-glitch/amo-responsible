    ...
    for (const oldLead of foundLeads) {
      ...
      if (ALLOWED_OLD_TYPES.includes(currentType)) {
        ...
        await fetch(`https://${env.AMO_DOMAIN}/api/v4/leads/${leadId}`, {
          ...
          body: JSON.stringify({ custom_fields_values: [{ field_id: FIELD_REQUEST_TYPE, values: [{ enum_id: NEW_TYPE_VALUE }] }] })
        });
        console.log(`✅ Дубль: сделка ${leadId} переведена в Сущ`);
        return;
      }
    }
  } catch (e) { /* Игнорируем */ }
      ...
      if (hasUpdate) {
        ...
        if (lead.pipeline_id === 5276629 && lead.status_id === 142) {
          if (!lead.name || !lead.name.toLowerCase().includes("исключение")) {
            ...
            const budgetUpdates = getBudgetUpdates(lead, fields, promo);
